...
const fieldUser = require('./field_user');
...
async function editProfile(req, res) {
    try {
        let info = {
            full_name: req.body.full_name ? req.body.full_name : req.user.full_name,
...
async function changePassword(req, res) {
    ...
    try {
        let [err] = validation(req.body, fieldUser.change_password);
        if(err) throw err;
...
async function changeAdmin(req, res){
    try {
        let [err] = validation(req.body, fieldUser.change_admin);
        if(err) throw err;
